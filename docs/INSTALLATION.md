...
Для GPC и Pharmadepot можно оставить CSS-селектор пустым: встроенный профиль подставит стабильный meta-селектор текущей цены. Если скидка добавляет на страницу старую зачёркнутую цену, профиль продолжает читать действующую цену. Для PSP достаточно выбрать метод PSP и указать URL товара.
